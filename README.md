# Frequency-aware Gradient Rectification (FGR) ✨

This repository contains the open-source implementation of **Frequency-aware Gradient Rectification (FGR)**, accepted by **ICML 2026**.

FGR trains with a small ratio of frequency-filtered samples and applies asymmetric projection-based gradient rectification to improve calibration under distribution shift.
...
```bash
bash train_scripts/cifar10/train.sh
```

Then evaluate the saved checkpoint:

```bash
bash evaluate_scripts/cifar10/evaluate.sh
```

The CIFAR scripts follow the original experiment-script style: each file contains multiple active commands. Comment out commands you do not want to run.
...
```bash
bash train_scripts/cifar100/train.sh
```
...
```bash
bash evaluate_scripts/cifar100/evaluate.sh
```
