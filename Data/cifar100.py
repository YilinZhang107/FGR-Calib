...
def get_train_valid_loader(
    ...
    train_full = datasets.CIFAR100(root=root, train=True, download=download, transform=transforms.Compose(train_ops))
    val_full = datasets.CIFAR100(root=root, train=True, download=False, transform=transforms.Compose(eval_ops))
...
def get_test_loader(root, batch_size, num_workers=4, pin_memory=True, download=False, input_size=None):
    ...
    dataset = datasets.CIFAR100(root=root, train=False, download=download, transform=transforms.Compose(ops))
